...
1. **Graph severed at almost every op.** The copy constructor deep-copied data
   ...
   consumer holds the _same_ graph node, and `GradFn::inputs` owns real handles.
...
3. **Double gradient propagation.** The engine iterated a topological list
   _and_ each backward recursively invoked its inputs' backwards, so gradients
...
5. **`MatMulBackward` computed wrong numbers.** It fed a _strided_ transpose
